...
class Masker(object):
    ...
    def __init__(self, band, **var):
        ...
        if type(band) is str:
            if 'band_num' in var:
                self.load_file(file_path=band, band_num=var['band_num'])
            elif 'band_name' in var:
                self.load_file(file_path=band, band_name=var['band_name'])
            else:
                self.load_file(file_path=band)
        else:
            self.load_data(file_path=band)

    def load_file(self, file_path, band_num = 0, band_name = 'QC_Day'):
        ...
        if extension == '.hdf':
            ...
            # load data using pyhdf rather than gdal (due to issue with hdf4)
            file = SD(file_path, SDC.READ)
            datasets_dic = file.datasets()
            if band_name not in datasets_dic.keys():
                print('please select band from blow')
                print(datasets_dic.keys())
                return
            # get data
            sds_obj = file.select(band_name) # select sds
            self.band_data = sds_obj.get() # get sds data
        else:
...
#
#     def __init__(self, file_path, qc_type='QC_Day'):
#         print('Loading QC for ', file_path)
#         if 'MOD11A2' in file_path:
#             if qc_type == 'QC_Day':
#                 super(ModisMasker, self).__init__(file_path, band_name='QC_Day')
#             elif qc_type == 'QC_Night':
#                 super(ModisMasker, self).__init__(file_path, band_name='QC_Night')
#         elif 'MOD09GQ' in file_path:
#             '''
#             needs to be filted
#             '''
